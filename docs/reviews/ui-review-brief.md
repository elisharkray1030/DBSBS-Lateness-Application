...
2. Start the app against a throwaway database: set `DB_PATH` to a path outside the repo (or under gitignored `data/`), run `python -m flask --app app init-db` once (seeds the Master List from root `namelist.csv`), then `python -m flask --app app run`. **Done when**: the Boarders tab lists the full roster. Never touch the real `lateness_history.db` at the repo root.
